...
// iso3166_1 holds the set of valid ISO 3166-1 alpha-2 country codes. Used by
// the checkout validator to reject "two random letters" inputs like "ZZ" that
// pass the cheaper `len=2,alpha` check but would never reach a real shipper.
//
// Source: ISO online browsing platform, March 2026 snapshot. Codes are
// reasonably stable but if Macedonia → North Macedonia-style renames happen
// the worst case is one extra failed checkout until we update.
var iso3166_1 = map[string]struct{}{
